...
// Nouveau nommage : "M{mois}_guide_moi.json" (numéro du mois dans le titre).
// Repli sur l'ancien nom "01-guide.json" pour les mois pas encore renommés.
function resolveGuidePath(dir) {
  const candidates = [`M${MOIS}_guide_moi.json`, "01-guide.json"];
...
async function main() {
  ...
  const path = resolveGuidePath(dir);
  console.log(`Import ciblé du module "guide" — mois ${m}\n`);

  if (!path) {
    console.error(`  ✗ Aucun fichier guide trouvé dans ${dir} (M${MOIS}_guide_moi.json ou 01-guide.json).`);
    process.exit(1);
  }
